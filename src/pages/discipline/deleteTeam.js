...
import { setDiscipline, setTeams } from "../../reducers/appSlice";
import { AddMessage } from "../../reducers/message/AddMessage";

function DeleteOther({ item, setItem }) {
  ...
  const confirme = async () => {
    ...
    const result = JSON.parse(await window.api.discipline.deleteother(item));

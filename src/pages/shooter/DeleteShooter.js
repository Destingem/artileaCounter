...
import { setShooter, setTeams } from "../../reducers/appSlice";
import { AddMessage } from "../../reducers/message/AddMessage";

function DeleteShooter({ item, setItem }) {
  ...
  const confirme = async () => {
    ...
    const result = JSON.parse(
      await window.api.people.delete({ _id: item._id })
    );
    ...
    const items = JSON.parse(await window.api.people.get());
    // if (items.err) return addMessage(items, dispatch);
    dispatch(setShooter(items.result));

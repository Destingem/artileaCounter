...
import { setShootingrange, setTeams } from "../../reducers/appSlice";
import { AddMessage } from "../../reducers/message/AddMessage";

function DeleteShootingRange({ item, setItem }) {
  ...
  const confirme = async () => {
    ...
    const result = JSON.parse(
      await window.api.shootingrange.delete({ _id: item._id })
    );
...
export default DeleteShootingRange;
